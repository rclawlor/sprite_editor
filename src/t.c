...
void draw_pixel(int *pixels, int pixel_x, int pixel_y, ColourButton *colour, int sprite[]);
void draw_sprite(int *pixels, Sprite *sprite, int sprite_sheet[]);
void draw_sprite_sheet(int *pixels, int x, int y, int sprite_sheet[]);
void sprite_select();
...
int main()
{
    ...
    FILE *sprite;
    int sprite_data[TILE_SIZE*TILE_SIZE];
    int sprite_sheet_data[TILE_SIZE*TILE_SIZE*N_TILES];
    Sprite sprite_sheet[N_TILES];

    for (int i=0; i<TILE_SIZE*TILE_SIZE*N_TILES; ++i)
    {
        sprite_sheet_data[i] = 20;
    }
    ...
    while (1)
    {
        ...
        if (mouse.click)
        {
            ...
            if (pixel_x>=0 && pixel_x<TILE_SIZE && pixel_y>=0 && pixel_y<TILE_SIZE)
            {
                ...
                draw_pixel(pixels, pixel_x, pixel_y, &current_colour, sprite_sheet);
            }
        }
...
void draw_pixel(int *pixels, int pixel_x, int pixel_y, ColourButton *colour, int sprite[])
{
    ...
    sprite[pixel_x + TILE_SIZE*pixel_y] = colour->colour_index;
};
    ...
    for (int i=0; i<2*TILE_SIZE; ++i)
    {
        for (int j=0; j<2*TILE_SIZE; ++j)
        {
            pixels[buffer_index + i + j*SCREEN_WIDTH] = palette[
                sprite_sheet[sprite->index*TILE_SIZE*TILE_SIZE + i/2 + (j/2)*TILE_SIZE]
                ];
        }
    }
}
...
void draw_sprite_sheet(int *pixels, int x, int y, int sprite_sheet[])
{
    int buffer_index = x + y*SCREEN_WIDTH;
    Sprite sprite = {
        .x = 0,
        .y = 0,
        .index = 0,
    ...
    for (int i=0; i<N_TILES; ++i)
    {
        sprite_x = 2*(i%TILE_SIZE)*TILE_SIZE;
        sprite_y = 2*(i/TILE_SIZE)*TILE_SIZE;
        draw_sprite(pixels, x+sprite_x, y+sprite_y, i, sprite_sheet);
    }
}